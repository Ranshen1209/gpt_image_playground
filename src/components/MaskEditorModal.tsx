...
function blobToDataUrl(blob: Blob): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    reader.onerror = () => reject(reader.error ?? new Error(i18n.t('mask.exportFailed')))
    ...
    async function loadCanvases() {
      try {
        ...
        if (preparedTarget.wasResized) {
          showToast(
            t('mask.imageResized', {
              originalWidth: preparedTarget.originalWidth,
              originalHeight: preparedTarget.originalHeight,
    ...
    <>
      ...
      <div className="flex-none flex items-center justify-between px-4 py-3 border-b border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-950 z-20">
          ...
          <div className="relative flex items-center gap-1.5">
            ...
            {showMaskInfo && (
              <div className="absolute left-0 top-full mt-2 w-64 rounded-xl border border-gray-200/80 bg-white px-3 py-2 text-xs leading-5 text-gray-600 shadow-lg dark:border-white/[0.08] dark:bg-gray-900 dark:text-gray-300">
                ...
                <p>{t('mask.infoLine1')}</p>
                <p className="mt-2">{t('mask.infoLine2')}</p>
              </div>
            )}
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleSave} disabled={!isReady || isSaving} className="flex h-8 items-center gap-1.5 px-4 text-sm font-medium text-white bg-[#9181bd] hover:bg-[#7d6cb0] rounded-lg disabled:opacity-50 transition">
            {isSaving ? t('mask.saving') : t('mask.save')}
          </button>
        </div>
      </div>
          ...
          <div className="flex items-center gap-2 sm:gap-4 px-2 sm:px-3 py-1.5 sm:py-2 bg-white/95 dark:bg-[#0f0f0f]/95 backdrop-blur-md border border-gray-200/80 dark:border-white/5 rounded-2xl sm:rounded-[1.25rem] shadow-2xl pointer-events-auto">
            <div className="flex items-center gap-1.5 sm:gap-3">
              <div className="flex items-center bg-gray-100/80 dark:bg-[#232325]/80 p-1 rounded-xl sm:rounded-[14px]">
                <button
                  className={`p-2 sm:p-2.5 rounded-lg sm:rounded-xl transition-all ${tool === 'brush' ? 'bg-white shadow-sm text-[#9181bd] dark:bg-[#323338] dark:text-[#c4b8e0] dark:shadow-none' : 'text-gray-500 hover:text-gray-700 dark:text-[#8a8a8e] dark:hover:text-gray-200'}`}
                ...
                <button
                  className={`p-2 sm:p-2.5 rounded-lg sm:rounded-xl transition-all ${tool === 'eraser' ? 'bg-white shadow-sm text-[#9181bd] dark:bg-[#323338] dark:text-[#c4b8e0] dark:shadow-none' : 'text-gray-500 hover:text-gray-700 dark:text-[#8a8a8e] dark:hover:text-gray-200'}`}
      ...
      {showBrushControls && sliderAnchor && createPortal(
        ...
        >
          <input
            ...
            className="absolute left-1/2 top-1/2 h-5 w-32 -translate-x-1/2 -translate-y-1/2 -rotate-90 accent-[#9181bd] cursor-ns-resize"
            disabled={!isReady || isSaving}
          />
        </div>,
        document.body,
