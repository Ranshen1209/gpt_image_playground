...
export default function DetailModal() {
  ...
  const taskProviderName = taskProvider ? 'Sakrylle' : t('common.unknown')
  ...
  const handleCopyError = async () => {
    const errorText = resolveErrorForDisplay(task.error, t('detail.generationFailed'))
    try {
      await copyTextToClipboard(errorText)
      showToast(t('detail.errorCopied'), 'success')
    } catch (err) {
      showToast(getClipboardFailureMessage(t('detail.errorCopyFailed'), err), 'error')
    }
  }
  ...
  const handleCopyPrompt = async () => {
    ...
    try {
      ...
      showToast(getClipboardFailureMessage(t('detail.promptCopyFailed'), err), 'error')
    }
  }
  ...
  const handleShowPromptWarning = () => {
    showCodexCliPrompt(
      true,
      currentRevisedPrompt ? t('errors.codexCliReasonRewritten') : t('errors.codexCliReasonMissing'),
    )
  }
  ...
  const handleCopyInputImage = async () => {
    ...
    try {
      ...
      console.error(err)
      showToast(getClipboardFailureMessage(t('detail.referenceCopyFailed'), err), 'error')
    }
  }
  ...
  const handleDownloadCurrentOutput = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (result.successCount === 0) {
        showToast(t('detail.downloadFailed'), 'error')
      } else {
        showToast(t('detail.downloadSuccess'), 'success')
      }
    } catch (err) {
      console.error(err)
      showToast(t('detail.downloadFailed'), 'error')
  ...
  const handleDownloadAllOutputs = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (result.successCount === 0) {
        ...
        showToast(t('detail.downloadPartialFailed', { success: result.successCount, fail: result.failCount }), 'error')
      } else {
        showToast(result.successCount > 1 ? t('detail.downloadSuccessCount', { count: result.successCount }) : t('detail.downloadSuccess'), 'success')
      }
    } catch (err) {
      console.error(err)
      showToast(t('detail.downloadFailed'), 'error')
  ...
  const handleDownloadPartialImages = async () => {
    ...
    try {
      ...
      if (result.successCount === 0) {
        ...
        showToast(t('detail.downloadSuccessPartial', { count: result.successCount }), 'success')
      }
    } catch (err) {
      console.error(err)
      showToast(t('detail.downloadFailed'), 'error')
    ...
    >
      ...
      >
        ...
        <div className="md:w-1/2 w-full h-64 md:h-auto bg-gray-100 dark:bg-black/20 relative flex items-center justify-center flex-shrink-0 min-h-[16rem]">
              ...
              {outputLen > 1 && (
                <div className="relative group flex">
                  <button
                    ...
                    aria-label={t('detail.downloadAll')}
                  >
                    <DownloadIcon className="h-4 w-4" />
              ...
              {task.status === 'running' && streamPreviewLen > 0 && (
                <>
                  ...
                  {(!currentStreamPreviewSrc || !streamPreviewLoaded) && (
                    <svg className="w-10 h-10 text-[#c4b8e0] animate-spin" fill="none" viewBox="0 0 24 24">
          ...
          {task.status === 'error' && isFalReconnecting && (
            <div className="w-full max-w-md px-4 text-center">
              ...
              <p className="text-sm font-medium text-yellow-500">{t('detail.reconnecting')}</p>
            </div>
          )}
          {task.status === 'error' && !isFalReconnecting && (
            <div className="w-full max-w-md px-4 text-center">
              ...
              >
                {resolveErrorForDisplay(task.error, t('detail.generationFailed'))}
              </p>
              <div className="mt-3 flex items-center justify-center gap-2">
                      ...
                      onClick={async (e) => {
                        if (task.rawImageUrls!.length === 1) {
                          ...
                          try {
                            ...
                            showToast(getClipboardFailureMessage(t('detail.linkCopyFailed'), err), 'error')
                          }
                        } else {
                          dismissAllTooltips()
                ...
                <div className="relative group">
                  <button
                    ...
                    className="inline-flex items-center justify-center rounded-full border border-[#d4c5ec]/80 bg-white/80 px-3 py-1.5 text-[#9181bd] transition hover:bg-[#f1edf8] dark:border-[#9181bd]/20 dark:bg-white/[0.04] dark:hover:bg-[#9181bd]/10"
        ...
        <div className="md:w-1/2 w-full p-5 overflow-y-auto overscroll-contain flex flex-col">
          ...
          <div data-selectable-text className="flex-1">
              ...
              {showPromptWarning && (
                <span className="relative inline-flex">
                  <button
                    ...
                    aria-label={t('detail.promptRewritten')}
                  >
                ...
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  {allInputImageIds.length > 0 && (
                    <button
                      ...
                      className="p-1 rounded text-gray-400 hover:bg-gray-100 dark:text-gray-500 dark:hover:bg-white/[0.06] transition"
                      title={t('detail.referenceCopied')}
                    >
                      <CopyIcon className="h-4 w-4" />
                    </button>
                  )}
                  ...
                  <>
                    ...
                    {isAgentEditTool && (
                      <div className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                        {t('detail.referenceMaybeMore')}
                      </div>
                    )}
                  </>
                ) : (
                  <div className="text-xs text-gray-500 dark:text-gray-400">
                    {t('detail.referenceModelChosen')}
                  </div>
                )}
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-2 text-xs mb-4">
              ...
              {task.params.output_compression != null && (
                <div className="bg-gray-50 dark:bg-white/[0.03] rounded-lg px-3 py-2">
                  <span className="text-gray-400 dark:text-gray-500">{t('detail.compression')}</span>
                  <br />
            ...
            <div className="text-xs text-gray-400 dark:text-gray-500 mb-4">
              ...
              {formatDuration() && <span> · {t('detail.duration', { duration: formatDuration() })}</span>}
            </div>
          </div>
          ...
          <div className="grid grid-cols-4 sm:flex gap-2 pt-4 border-t border-gray-100 dark:border-white/[0.08]">
            <button
              onClick={handleReuse}
              className="col-span-2 sm:flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl bg-[#f1edf8] dark:bg-[#9181bd]/10 text-[#7d6cb0] dark:text-[#c4b8e0] hover:bg-[#e4d9f5] dark:hover:bg-[#9181bd]/20 transition text-sm font-medium whitespace-nowrap"
            >
              <svg className="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" />
              </svg>
              {t('detail.reuseConfig')}
            </button>
            <button
              onClick={handleEdit}
              disabled={!outputLen}
            ...
            <button
              ...
              title={task.isFavorite ? t('detail.unfavorite') : t('detail.favorite')}
            >
        ...
        >
          <div ref={rawUrlsModalRef} className="flex w-full max-w-2xl max-h-[90vh] flex-col overflow-hidden rounded-2xl bg-white shadow-xl dark:bg-[#1c1c1e]" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4 dark:border-white/[0.08] shrink-0">
              <h3 className="text-base font-semibold text-gray-900 dark:text-white">{t('detail.rawUrlsTitle', { count: rawImageUrls.length })}</h3>
                ...
                <button
                  ...
                  onClick={async () => {
                    try {
                      ...
                      showToast(t('detail.copySuccess'), 'success')
                    } catch (err) {
                      showToast(getClipboardFailureMessage(t('detail.copyFailed'), err), 'error')
                    }
                  }}
      ...
      {showRawResponseModal && task?.rawResponsePayload && (
        ...
        >
          ...
          >
            <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4 dark:border-white/[0.08] shrink-0">
              <h3 className="text-base font-semibold text-gray-900 dark:text-white">{t('detail.rawResponseTitle')}</h3>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={async () => {
