...
export default function InputBar() {
  ...
  const handleToggleFavorite = useCallback(() => {
    ...
    setConfirmDialog({
      ...
      confirmText: newFavoriteState ? t('input.batchFavoriteConfirm') : t('input.batchUnfavoriteConfirm'),
      action: () => {
    ...
    <div className={`grid ${cols} gap-2 text-xs flex-1`}>
      ...
      >
        ...
        <ButtonTooltip
          ...
          text={t('input.qualityDisabledCodexCli')}
        />
      </label>
      ...
      >
        ...
        <input
          ...
          min={0}
          max={100}
          placeholder={t('input.compressionPlaceholder')}
        ...
        <ButtonTooltip
          ...
          text={t('input.compressionDisabledTip')}
        />
      </label>
      <label
        className="relative flex flex-col gap-0.5"
      ...
      >
        ...
        <ButtonTooltip visible={streamConcurrentByN && !nLimitHint.visible} text={t('input.concurrentSplitTip')} />
      </label>
    </div>
  )
        ...
        <div className="fixed inset-0 z-[100] bg-white/60 dark:bg-gray-900/60 backdrop-blur-md flex flex-col items-center justify-center pointer-events-none">
          <div className="flex flex-col items-center gap-4 p-8 rounded-3xl">
            <div className={`w-20 h-20 rounded-full border-2 border-dashed flex items-center justify-center ${
              atImageLimit ? 'bg-red-50 dark:bg-red-500/10 border-red-300' : 'bg-[#f1edf8] dark:bg-[#9181bd]/10 border-[#9181bd]'
            }`}>
              {atImageLimit ? (
            ...
            <div className="text-center">
              {atImageLimit ? (
                <>
                  <p className="text-lg font-semibold text-red-500">{t('input.limitReached', { limit: API_MAX_IMAGES })}</p>
                  <p className="text-sm text-gray-400 mt-1">{t('input.removeSomeFirst')}</p>
                </>
              ) : (
                <>
                  <p className="text-lg font-semibold text-gray-700 dark:text-gray-200">{t('input.releaseToUpload')}</p>
          ...
          <div className="flex justify-center mb-3">
            <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur shadow-[0_8px_30px_rgb(0,0,0,0.12)] dark:shadow-lg rounded-full flex items-center p-1 border border-gray-200/50 dark:border-white/10 pointer-events-auto">
              ...
              <button
                ...
                className="p-2 text-[#9181bd] dark:text-[#c4b8e0] hover:text-[#7d6cb0] dark:hover:text-[#c4b8e0] transition-colors"
                title={selectedTaskIds.length === filteredTasks.length && filteredTasks.length > 0 ? t('input.deselectAll') : t('input.selectAll')}
              >
