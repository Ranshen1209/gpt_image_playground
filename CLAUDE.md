...
| 模式 | 路径（相对 baseUrl） | 用途 |
|---|---|---|
| Images API | `POST images/generations` | 文生图（默认 `stream:true` + `partial_images` 心跳防 524） |
...
- **流式 chat/completions 图像路径**(v0.10.3 引入,v0.11.x **已移除**): 曾在 Sakrylle baseUrl + `streamChatCompletionsImage`(默认 true)时,把文生图/参考图/蒙版统一走 `POST chat/completions` `stream:true` 防 Cloudflare 524。**该开关与整条链路已删除**(`streamChatCompletionsImage` 字段、`shouldUseChatImagePath`、`chatCompletionsImageApi.ts`、locale `streamChatImage*`/`timeoutHintChatOverall`、oauthFallback 的 `chat.completions:create` scope 分支全删)。现统一靠「流式传输」(`streamImages`,见下条):走 `images/generations|edits` `stream:true` + `partial_images` 心跳维持连接防 524。`getDefaultStreamImages` 改为 openai 任意模式默认 `true`,`isDefaultOpenAIProfile` 的 untouched 判定同步改 `streamImages === true`。**注意**:CLAUDE.md 旧结论曾记「images/generations 即使 stream:true 也返回整块 JSON、零字节触发 524」——这与 `streamImages`+`partial_images` 心跳机制存在张力,上线后需观察慢图是否仍触发 524;若复发,考虑恢复 chat 路径或调高 `partial_images`。
